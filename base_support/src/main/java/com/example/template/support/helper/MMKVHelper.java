...
import com.example.template.support.utils.JsonUtil;
...
public class MMKVHelper {
    ...
    private volatile static MMKVHelper instance = null;
    ...
    public void init(Context context) {
        mContext = context;
        //mmkv默认存储路径getFilesDir().getAbsolutePath() + "/mmkv"
    ...
    public MMKV getMMKV(String name) {
        ...
        if (TextUtils.isEmpty(name)) {
            mmkv = encrypt ? MMKV.defaultMMKV(MMKV.MULTI_PROCESS_MODE, cryptKey) :
                    MMKV.defaultMMKV(MMKV.MULTI_PROCESS_MODE, null);
        } else {
            mmkv = encrypt ? MMKV.mmkvWithID(name, MMKV.MULTI_PROCESS_MODE, cryptKey) :
    ...
    public void putObjectData(Serializable serializable) {
        getMMKV().putString(serializable.getClass().getName(),
                        JsonUtil.parserObjectToGson(serializable))
                .apply();
    }

    public void putObjectData(String kvName, Serializable serializable) {
        getMMKV(kvName).putString(serializable.getClass().getName(),
                        JsonUtil.parserObjectToGson(serializable))
                .apply();
    }
